...
from model.networks.unet import UNetAttention, UNet
from model.networks.segformer import Segformer
...
def network_initializer(network, device):
    ...
    :return: Network
    """
    if network == "unet_attention":
        print("You are using Attention UNet")
        Network = UNetAttention
    elif network == "unet":
        print("You are using UNet")
        Network = UNet
    elif network == "segformer":
        print("You are using SegFormer")
        Network = Segformer
    elif network == "unet_light":
        print("You are using light UNet")
        Network = UNetLight
...
def optimizer_initializer(model, optim, init_lr, device):
    ...
    if type(model) is list :
        model_param = list(model[0].parameters()) + list(model[1].parameters())
    else :
...
def sample_initializer(sample, image_size, device, sample_step=100):
    """
    Initialize sample
    ...
    if sample == "ddpm":
        ...
        diffusion = DDIMDiffusion(sample_steps=sample_step, img_size=image_size, device=device)
    else:
        diffusion = DDPMDiffusion(img_size=image_size, device=device)
