...
class DDIMDiffusion(BaseDiffusion):
    ...
    def sample_seg(self, model, img,labels=None, cfg_scale=None):
        """
        DDIM sample method
        :param model: Model
        ...
        with torch.no_grad():
            ...
            for i, p_i in tqdm(self.time_step):
                ...
                concat_tensor = torch.cat([x, img], dim=1)
                predicted_noise = model(concat_tensor, t)
                # Calculation formula
    ...
    def sample_rec(self, model, ynet, img_target,labels=None, cfg_scale=None):
        """
        DDIM sample method
        :param model: Model
        :param n: Number of sample images
        ...
        with torch.no_grad():
            ...
            for i, p_i in tqdm(self.time_step):
                ...
                # Whether the network has conditional input, such as multiple category input
                # Images and time steps input into the model
                concat_target = torch.cat([x, img_target], dim=1)
                latent = model.forward_latent(concat_target,t)
                recons_output = ynet(latent[0],latent[1],latent[2],latent[3],latent[4])
        model.train()
        ynet.train()
        recons_output = (recons_output * 255).type(torch.uint8)
